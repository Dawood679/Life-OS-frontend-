...
export default function VerifyForgotOtp() {
  ...
  const handleChange = (index, value) => {
    ...
    const newOtp = [...otp];
    newOtp[index] = value.slice(-1); // Take only the latest digit typed
    setOtp(newOtp);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Navigate to Reset Password screen with state
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-slate-100 via-slate-200 to-slate-400 p-4 font-sans">
      ...
      <div className="bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl overflow-hidden w-[90vw] max-w-6xl min-h-[82vh] grid grid-cols-1 md:grid-cols-2 border border-white/60">
        ...
        {/* LEFT SECTION: Pure CSS/SVG LifeOS Passcode Verification Stage */}
        <div className="bg-gradient-to-b from-blue-50/80 via-pink-50/40 to-indigo-50/60 p-8 hidden md:flex md:flex-col md:justify-between md:items-center text-center border-r border-slate-100 relative overflow-hidden">
          ...
          <div className="w-full my-auto flex flex-col items-center justify-center relative z-10 py-6">
            <div className="relative w-72 h-72 flex items-center justify-center">
              ...
              <div className="absolute top-2 left-2 bg-white/90 backdrop-blur-md border border-slate-200/80 shadow-md px-3.5 py-2 rounded-xl flex items-center gap-2 animate-bounce [animation-duration:3.8s]">
                <span className="w-6 h-6 rounded-lg bg-emerald-100 text-emerald-600 flex items-center justify-center text-xs font-bold">
                  ✓
                </span>
                <div className="text-left">
                  <p className="text-[9px] text-slate-400 font-medium leading-none">Code Sent</p>
                  <p className="text-xs font-semibold text-slate-700 mt-0.5">Inbox Ready</p>
                </div>
              </div>
              ...
              <div className="absolute bottom-2 right-2 bg-white/90 backdrop-blur-md border border-slate-200/80 shadow-md px-3.5 py-2 rounded-xl flex items-center gap-2">
                ...
                <div className="text-left">
                  <p className="text-[9px] text-slate-400 font-medium leading-none">Passcode</p>
                  <p className="text-xs font-semibold text-slate-700 mt-0.5">Single-Use 2FA</p>
                </div>
              </div>
                ...
                >
                  {otp.map((digit, index) => (
                    <Input
                      ...
                      inputClassName="w-11 h-12 md:w-12 md:h-14 text-center font-bold text-xl text-slate-800 bg-orange-50/70 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-400/50 focus:border-indigo-400 shadow-sm transition-all p-0"
                    />
                  ))}
                </div>
              </div>
